...
仓库包含固定腾讯来源、SHA-256 和 Authenticode 门控的 4.1.9.57 备用捕获器。它不是默认流程，也不会在 `capture` 失败后自动执行。只有用户分别批准下载、准备、程序切换、内存读取和恢复后才可使用；完整命令和回滚顺序见 [Windows 4.1.9 备用流程](emergency-downgrade.md)。
